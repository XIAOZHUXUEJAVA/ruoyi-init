<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="qualityList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="污染物排放情况" align="center" prop="pollutantEmission" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="空气质量指数" prop="airQuality">
          <el-input v-model="form.airQuality" placeholder="请输入空气质量指数" />
        </el-form-item>
        <el-form-item label="噪音水平" prop="noiseLevel">
          <el-input v-model="form.noiseLevel" placeholder="请输入噪音水平" />
        </el-form-item>
        <el-form-item label="污染物排放情况" prop="pollutantEmission">
          <el-input v-model="form.pollutantEmission" placeholder="请输入污染物排放情况" />
        </el-form-item>
        <el-form-item label="记录日期" prop="recordDate">
          <el-date-picker clearable
...
<script>
import { listQuality, getQuality, delQuality, addQuality, updateQuality } from "@/api/city-data/quality";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除城市环境质量编号为"' + envIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('city-data/quality/export', {
        ...this.queryParams
